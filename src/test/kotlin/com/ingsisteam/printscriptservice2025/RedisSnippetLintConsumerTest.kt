...
import com.ingsisteam.printscriptservice2025.redis.consumer.SnippetLintConsumer
...
class RedisSnippetLintConsumerTest {
    ...
    private lateinit var consumer: SnippetLintConsumer

    @BeforeEach
    fun setUp() {
        redisMock = mock()
        serviceMock = mock()
        consumer = SnippetLintConsumer(redisMock, "test-stream", "test-group", serviceMock)
    }

    @Test
    fun `onMessage should process record and call lintSnippet`() {
        // Arrange
        ...
        // Assert
        verify(serviceMock).lintSnippet(snippet)
    }
    ...
    fun `onMessage should handle exceptions gracefully`() {
        ...
        val snippet = Snippet("exampleId", "exampleContent", "exampleUserId", UUID.randomUUID())
        val record = ObjectRecord.create("test-stream", snippet)
        whenever(serviceMock.lintSnippet(any())).thenThrow(RuntimeException("Test exception"))
