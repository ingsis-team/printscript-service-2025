...
constructor(
    ...
    suspend fun publishEvent(snippet: Snippet) {
        logger.info("Publishing format event for snippetId: ${snippet.id}, userId: ${snippet.userId}, correlationId: ${snippet.correlationID}")
        emit(snippet).awaitSingle()
        logger.debug("Format event published successfully to stream: $streamKey")
    }
}
